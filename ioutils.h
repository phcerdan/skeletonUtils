...
int readImageInfo(std::string filename, itk::ImageIOBase::IOComponentType *ComponentType)
{
...
template <class TImage>
typename TImage::Pointer readImOrient(std::string filename)
{
  typedef typename itk::ImageFileReader<TImage> ReaderType;
  typename ReaderType::Pointer reader = ReaderType::New();

  typedef typename itk::OrientImageFilter<TImage, TImage> OrientType;
  ...
  typename TImage::Pointer result = orienter->GetOutput();
  try
    {
    result->Update();
    }
